...
def watermark_raw_file(src_path, dest_path):
    ...
    Supports Windows long-path prefix.
    """
    try:
        long_src = '\\\\?\\' + os.path.abspath(src_path) if sys.platform == "win32" and not src_path.startswith('\\\\?\\') else src_path
        with Image.open(long_src) as raw:
...
def main():
    ...
    print("=================================================================")
    print("   TIẾN TRÌNH ĐÓNG DẤU CHUẨN 1 WATERMARK CHO ẢNH GỐC 'ẢNH WEB ĐỒ ĐỒNG'   ")
    print("=================================================================")

    with open('scratch/sync_plan.json', 'r', encoding='utf-8') as f:
        plan = json.load(f)
    ...
    for item in plan['new_products']:
        ...
        p_slug = item['final_slug']
        for idx, src in enumerate(item['detected']['images'], 1):
            dest = os.path.join('public', 'images', 'products', cat_slug, f'{p_slug}-{idx}.jpg')
            raw_tasks.append((src, dest))

    # 2. Matched products (new angles)
    for item in plan['exact_matches']:
        cat_slug = item['cat_slug']
        db_slug = item['db_product']['slug']
        for idx, src in enumerate(item['detected']['images'], 1):
            dest = os.path.join('public', 'images', 'products', cat_slug, f'{db_slug}-goc-{idx}.jpg')
            raw_tasks.append((src, dest))
    ...
    print(f"\n=================================================================")
    print(f"HOÀN TẤT ĐÓNG DẤU ẢNH GỐC TRONG {elapsed:.1f} GIÂY!")
    print(f"Thành công: {success_raw}/{len(raw_tasks)} ảnh.")
    print("=================================================================")
