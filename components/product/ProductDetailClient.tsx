...
function renderFormattedInline(text: string) {
  ...
  return parts.map((part, i) => {
    if (part.startsWith("**") && part.endsWith("**")) {
      return (
        <strong key={i} className="text-[#ffd700] font-semibold">
          {part.slice(2, -2)}
        </strong>
      );
    }
...
function ProductStructuredDescription({
  description,
  productName,
}: {
  description?: string | null;
    ...
    <div className="space-y-4">
      {rawSections.map((sec, secIdx) => {
        ...
        return (
          <div
            key={secIdx}
            className="rounded-xl bg-[#070e17]/85 border border-[#1c2c3d] p-4 sm:p-5 shadow-sm hover:border-[#dfb755]/50 transition-colors"
          >
            {headerText && (
              <div className="flex items-center justify-between gap-3 pb-3 mb-3 border-b border-[#1c2c3d]">
            ...
            {paragraphs.length > 0 && (
              <div className="space-y-2.5 text-xs sm:text-sm text-[#cbd5e1] leading-relaxed mb-3">
            ...
            {listItems.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs sm:text-sm text-[#cbd5e1] pt-1">
                {listItems.map((item, itIdx) => (
                  <div
                    key={itIdx}
                    className="flex items-start gap-2.5 p-3 rounded-lg bg-[#0b1422]/80 border border-[#1c2c3d]/60"
                  >
